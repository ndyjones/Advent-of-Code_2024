...
# add(+) and multiply(*) and NOW || concatination are the only possible operators to go between eq_values to produce test_value
...
def evaluate_recursively(values, index, current_result, test_value):
    ...
    if evaluate_recursively(values, index + 1, current_result * next_value, test_value):
        return True
    # concatenate (||)
    concatenated_value = int(str(current_result) + str(next_value))
    if evaluate_recursively(values, index + 1, concatenated_value, test_value):
        return True
